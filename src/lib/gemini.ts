...
function searchQuotaExhaustedError(): GeminiError {
  return new GeminiError(
    "Gemini 的「联网搜索」额度用完了——注意这跟模型额度是分开算的，换模型没用（普通的 AI 功能这会儿照样能用）。Google 免费版给联网搜索的额度非常少，需要等明天恢复，或者配一个 Qwen / Claude / OpenAI 的 Key 来跑这个功能。"
  );
}
...
function quotaExhaustedError(triedModels: string[]): GeminiError {
  return new GeminiError(
    triedModels.length > 1
      ? `你配置的这几个 Gemini 模型（${triedModels.join("、")}）今天的免费额度都用完了（Google 按模型每天限量，明天会自动恢复）——这跟你配的其他服务商无关，简历体检/岗位匹配这几个功能必须用 Gemini（要读文件，其他服务商做不到），暂时没法切走`
      : `你这个 Gemini 模型（${triedModels[0]}）今天的免费额度用完了（Google 按模型每天限量，明天会自动恢复）——可以去账号设置里给这个 Key 的"模型"填多个、用逗号分隔（比如 gemini-3.5-flash-lite, gemini-3.6-flash），额度用完会自动换下一个。这跟你配的其他服务商无关，简历体检/岗位匹配这几个功能必须用 Gemini（要读文件，其他服务商做不到），暂时没法切走`
  );
}
 ...
 * Search before answering, so it can find real current information instead
 * of guessing from training data. Gemini's API doesn't allow combining
 ...
 * schema-enforced pass over this text instead of parsing free text
 * themselves.
 */
